    ...
    if not opt.video:     #处理图片
        if not os.path.isfile(opt.image_path):            #目录
            ...
            for img_path in file_list:
                ...
                # print(dict_list)
                ori_img, result_str=draw_result(img,dict_list)
                ...
                content_json = []
                content_dic = {
                    "name": opt.output + '/' + img_name,
                    "result": result_str,
                }
                content_json.append(content_dic)
                # 将 json 数据写入文件
                with open(opt.output + '/' + img_name + '.json', 'w', encoding='utf-8') as f:
        ...
        save_video_path = opt.output + '/' + opt.video.split('/')[-1]+'_result.mp4'
        out = cv2.VideoWriter(save_video_path, fourcc, fps, (width, height))  # 写入视频
        frame_count = 0
        fps_all=0
